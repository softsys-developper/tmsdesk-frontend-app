    ...
    <template v-slot:form>
      <div class="w-full space-y-2">
        <div class="" v-for="fr in PaidForms">
          {{ fr.value }}
          <InForm :title="fr.label" :name="fr.name" :value="fr.value" :label="fr.label" :type="fr.type"
            ...
            <div v-for="(Primes, index) in useDataStore().Primes">
              <div class="flex items-start ">
                <div class="mb-[0.5rem] block min-h-[1.5rem] ps-[1.5rem]">
                  <input type="checkbox" :id="'checkbox-' + index" :value="Primes.id"
...
//   if (useUpdateStore().isUpdate.is) {
//     console.log(useUpdateStore().isUpdate.data)
//     PaidForms[0].value = useUpdateStore().isUpdate.data.id
//   }
// })
...
onMounted(() => {
  ...
  readData(API_URL.BANQUE_LIST).then((data) => {
    ...
    remplacerObjetDansTableau(PaidForms, 'name', '', data.datas)
  })
