...
export const useStagiaireHook = () => {
  ...
  const formatStagiaireData = (Stagiaires: any) => {
    return Stagiaires.map((Stagiaire: any) => ({
      ...
      image: `<img src="${
        Stagiaire.photo
          ? `https://api.usimeca.ci/stagiaires/photos/${Stagiaire.photo}`
  ...
  const CreateStagiaire = async (
    ...
    createData(API_URL.STAGE_CREATE, values)
      .then((data: any) => {
  ...
  const StagiaireUpdate = (id: any, values: any, callback: any) => {
    setService(
      ...
      formatStagiaireData,
      callback
    ).SetUpdate(API_URL.STAGE_UPDATE, id, values);
  };
  ...
  const StagiaireDelete = (id: any) => {
    setService(
      ...
      "Stagiaires",
      formatStagiaireData
    ).SetDelete(API_URL.STAGE_REMOVE, id);
  };
